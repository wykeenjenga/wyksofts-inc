    ...
    <main className="portal-page admin-page">
      ...
      <section className="admin-login-wrap">
        ...
        <div className="admin-login-card">
          {state === "loading" && <div><span className="admin-lock">W</span><h2>Checking link…</h2><p>We are securely validating your recovery session.</p></div>}
          {state === "setup" && <div><span className="admin-lock">W</span><h2>Recovery unavailable</h2><p>The authentication connection is not configured.</p></div>}
          {state === "invalid" && <div><span className="admin-lock">W</span><h2>Request a reset link</h2><p>Open this page from the password-reset email. Recovery links are single-use and expire for your protection.</p><a className="portal-submit" href="/admin/">Return to admin sign in <span>↗</span></a></div>}
          {state === "success" && <div><span className="admin-lock">✓</span><h2>Password updated</h2><p>Your new password is ready. You can now sign in to the WykSofts dashboard.</p><a className="portal-submit" href="/admin/">Continue to sign in <span>↗</span></a></div>}
          {state === "ready" && <form onSubmit={updatePassword}><span className="admin-lock">W</span><h2>Choose a new password</h2><p>Use at least 12 characters and avoid reusing a password from another account.</p><label>New password<input name="password" type="password" autoComplete="new-password" minLength={12} required /></label><label>Confirm new password<input name="confirmation" type="password" autoComplete="new-password" minLength={12} required /></label>{errorMessage && <p className="portal-error" role="alert">{errorMessage}</p>}<button className="portal-submit" type="submit" disabled={saving}>{saving ? "Updating password…" : "Set new password"}<span>↗</span></button></form>}
        </div>
      </section>
    </main>
  );
